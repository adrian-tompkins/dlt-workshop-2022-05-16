...
CREATE STREAMING LIVE TABLE bronze_sales_dlt
...
CREATE STREAMING LIVE TABLE silver_sales_dlt (
  CONSTRAINT `Location has to be 5 characters long` EXPECT (length(store_id) = 5) ON VIOLATION FAIL UPDATE,
  CONSTRAINT `Only CANCELED and COMPLETED transactions are allowed` EXPECT (order_state IN ('CANCELED', 'COMPLETED')) ON VIOLATION DROP ROW
) 
...
CREATE STREAMING LIVE TABLE silver_sale_items_dlt (
